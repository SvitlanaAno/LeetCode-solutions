...
https://www.algoexpert.io/questions/Run-Length%20Encoding
  ...
  example, the run "AAAAAAAAAAAA"  (12As), can't  naively be encoded as "12A",
...
public class RunLengthEncoding {
    ...
    static public String runLengthEncoding(String string) {
        ...
        // so our count start from 1.
        StringBuilder encodedStr = new StringBuilder();
        int count = 1;

        for(int i = 0 ; i < string.length() ; i++){
            while(i < string.length() -1 && count < 9 && string.charAt(i) == string.charAt(i+1)){
                count++;
                i++;
            }
            encodedStr.append((char)(count +'0'));
